...
import random, search_problem.state as st
...
class Maze:
    def __init__(self):
        ...
        # Starting from a grid, the method randomly generates a rectangular maze by applying depth-first
        # backtracking search
        def backtrack(x, y):
            self.maze[x][y] = 0
        ...
        # Create the initial list of goal states
        self.goal_nodes_list = [self.root_node] + self.intermediate_nodes_goal + [self.goal_node]
        self.goal_nodes_list.sort() # Sort the goal nodes by increasing associated coordinates
    ...
    # Method to process input variables using the State Design Pattern
    ...
    # Method to verify if the current state is the Goal State ---> Goal test
