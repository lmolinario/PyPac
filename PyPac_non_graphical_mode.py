from search_problem import search_strategy as s
from non_graphical_mode import performance_NG,maze_problem_NG as m
from graphical_mode import plotting
from config import configuration as c


def generate_maze_instances(num_instances):
    """Generate a list of random maze instances."""
    problem_instances = []
...
def main():
    ...
    for i in range(len(problem)):
        ...
        for algorithm, strategy in strategies.items():  # Use items() for both key and value
            p = performance_NG.Performance(algorithm, problem[i])
